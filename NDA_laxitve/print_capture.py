...
def pprint_enhanced_packet(block: EnhancedPacket):
    # text = [
...
def format_packet_information(packet_data) -> List[str]:
    decoded = Ether(packet_data)
    return format_scapy_packet(decoded)

last_ack = 0
def on_tcp(packet) -> bool:
    global last_ack
    if last_ack == packet.fields['ack']:
        print("dup ack:" , packet.fields['ack'])
        return True

    last_ack = packet.fields['ack']
    return True

def format_scapy_packet(packet) ->List[str]:
    result = []
    ...
    if (packet.__class__.__name__ == "TCP"):
        need_print = on_tcp(packet)
    
    if need_print:
        result.append("{0} {1}".format(col256(packet.__class__.__name__, "501"), " ".join(fields)))
...
def main():
    with open("cubic_cap.pcapng", "rb") as fp:
